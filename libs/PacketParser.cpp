...
std::string PacketParser::ParsePacket(std::string packet)
{
    ...
    if (IseCPRIPacket(packet.substr(((20 * 2) - 1), (2 * 2)))) // Type field is bytes number 21:23 (characters 41:45)
    {
        // Create eCPRI packet
        ...
        CurrentDataPacket->SetDataField(PacketDataFields::MessageType, packet.substr(((23 * 2) - 1), (1 * 2)));       // Byte 24
        CurrentDataPacket->SetDataField(PacketDataFields::PayloadSize, packet.substr(((24 * 2) - 1), (2 * 2)));       // Bytes 25:26
        CurrentDataPacket->SetDataField(PacketDataFields::ProtocolVersion, packet.substr(((22 * 2) - 1), (1 * 1)));     // Left Nibble of Byte 23
        CurrentDataPacket->SetDataField(PacketDataFields::RTCID, packet.substr(((26 * 2) - 1), (2 * 2)));         // Bytes 27:28
        CurrentDataPacket->SetDataField(PacketDataFields::SequenceID, packet.substr(((28 * 2) - 1), (2 * 2)));    // Bytes 29:30
        ...
        std::string last_bit = std::format("{:x}", (byte_23 % 2)); // int to hex string
